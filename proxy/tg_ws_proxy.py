...
def get_stats_for_tray() -> dict:
    """Return dict with 'tooltip' and 'menu' strings for tray UI."""
    s = _stats
    total = s.active_ws + s.active_tcp_fallback
    return {
        "tooltip": f"{total} подкл. · ↑ {_human_bytes(s.bytes_up)} ↓ {_human_bytes(s.bytes_down)} · WS",
        "menu": f"Сейчас: {s.active_ws} через WS, {s.active_tcp_fallback} через TCP",
    }


async def check_dc_connections(dc_opt: Dict[int, str],
                               timeout: float = 8.0) -> List[dict]:
    """
    Test WebSocket connectivity to each configured DC.
    Returns a list of dicts: {dc, status: "ws"|"tcp_fallback", ms?: int, error?: str}.
    """
    result: List[dict] = []
    for dc in sorted(dc_opt.keys()):
        ...
        if not ip:
            continue
        domains = _ws_domains(dc, False)
        last_error: Optional[str] = None
        success = False
        for domain in domains:
            t0 = time.monotonic()
            try:
                ws = await RawWebSocket.connect(ip, domain, timeout=timeout)
                ms = round((time.monotonic() - t0) * 1000)
                try:
                    await ws.close()
                except Exception:
                    pass
                result.append({"dc": dc, "status": "ws", "ms": ms})
                success = True
                break
            except WsHandshakeError as exc:
                last_error = "redirect" if exc.is_redirect else exc.status_line or str(exc)
                if not exc.is_redirect:
                    break
            except Exception as e:
        ...
        if not success:
            result.append({
                "dc": dc,
                "status": "tcp_fallback",
                "error": last_error or "timeout/error",
            })
    return result
